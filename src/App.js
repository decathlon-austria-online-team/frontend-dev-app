...
function App() {
  ...
  const [userEmailFitOneOfEmailsInSheetBool, setUserEmailFitOneOfEmailsInSheetBool] = useState(false);
  const [userDataObjFromSheet, setUserDataObjFromSheet] = useState({});
  const [userHasAllDataInSheetBool, setUserHasAllDataInSheetBool] = useState(false);
  ...
  return (
    ...
    {isAuthenticated && user.email.indexOf('@decathlon.com') === -1 ? (
      <NotDecaEmail />
    ) : isAuthenticated && user.email.indexOf('@decathlon.com') > -1 && userEmailFitOneOfEmailsInSheetBool && sheetApiDoneLoading ? (
              ...
              <CSSTransition
                ...
                >
                  <Switch location={location}>
                    <Route path="/" exact>
                      <Home userHasAllDataInSheetBool={userHasAllDataInSheetBool} />
                    </Route>
    ...
    ) : isAuthenticated && sheetApiDoneLoading && !userEmailFitOneOfEmailsInSheetBool ? (
